...
class User {
  ...
  //   } catch(err) {
  //     const { errmsg } = err
  //     if ( errmsg.includes('duplicate') ) return res.json({ error: 'duplicate email' })
  //     return res.json(err)
  //   }
  // }
  ...
  async register(req, res) {
    ...
    try {
      ...
      if ( user ) return res.json({ error: `${ email } previously registered.` })
      ...
      if ( errmsg.includes('duplicate' )) return res.json({ error: 'duplicate email err' })
      return res.json(err)
    }
  }
  async login(req, res) {
